...
class AttentionEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return attention_map * x.unsqueeze(2).unsqueeze(3)
...
class VAE(nn.Module):
    def __init__(self, img_dim=32, image_channels=3, h_dim=1024, z_dim=128, device=None):
        ...
        self.attention_encoder = AttentionEncoder(256, 256)  # Attention module for encoder
        self.attention_decoder = AttentionDecoder(h_dim, h_dim)  # Attention module for decoder
    ...
    def forward(self, x): # forward pass, maps the input features x to the reconstructed features
        h = self.encoder(x) # encodes the input features x into the hidden representation h
        z, mu, logvar = self.bottleneck(h) # combines the mean and the variance of the latent Gaussian distribution
         # Reshape h to match the expected input shape for AttentionEncoder
        h_reshaped = h.unsqueeze(2).unsqueeze(3)
        attention_encoder_output = self.attention_encoder(h_reshaped)
        attention_decoder_output = self.attention_decoder(self.fc3(z))

        reconstructed_x = self.decoder(attention_decoder_output)

        return reconstructed_x, z, mu, logvar, attention_encoder_output, attention_decoder_output
